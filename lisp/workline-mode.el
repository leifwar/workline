...
(defun workline-show-refresh ()
  ...
  (workline-section-apply
   (list (cons 'gitlab 'workline-gitlab-section) (cons 'github 'workline-github-section))))

(defun workline-web-job-trace-at-point ()
  (interactive)
  (workline-section-fun-at-point
   (list
    (cons 'gitlab 'workline-job-web-trace-at-point-gitlab)
...
(defun workline-job-trace-at-point ()
  ...
  (workline-section-fun-at-point
   (list
    (cons 'gitlab 'workline-job-trace-at-point-gitlab)
...
(defun workline-retry-job-at-point ()
  ...
  (workline-section-fun-at-point
   (list
    (cons 'gitlab 'workline-retry-job-at-point-gitlab)
...
(defun workline-cancel-job-at-point ()
  ...
  (workline-section-fun-at-point
   (list
    (cons 'gitlab 'workline-cancel-job-at-point-gitlab)
...
(defun workline-delete-job-at-point ()
  ...
  (workline-section-fun-at-point
   (list
    (cons 'gitlab 'workline-delete-at-point-gitlab)
...
Current branch only if optional FORCE-BRANCH-OPTION is given."
  (if-let ((remote-branch (magit-remote-branch-at-point)))
    (if (or force-branch-option (transient-arg-value "--branch" (transient-args 'workline-gitlab)))
...
(defun workline-trigger-pipeline ()
  ...
  (if-let ((repo (forge-get-repository :valid?)))
    (if (forge-gitlab-repository--eieio-childp repo)
        (workline-gitlab-trigger-pipeline repo (workline-branch-option t))
...
(defvar-keymap workline-mode-map
  ...
  "RET"
  'workline-job-trace-at-point
  "C-<return>"
  'workline-web-job-trace-at-point
  "v"
  'workline-job-trace-at-point
  "R"
  'workline-show-refresh
  "r"
  'workline-retry-job-at-point
  "c"
  'workline-cancel-job-at-point
  "d"
  'workline-delete-job-at-point)
